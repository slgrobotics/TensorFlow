...
class SquareBoxCamera:
    ...
    def run(self, x, y, box_size=None, color=None):
        ...
        frame[self.clamp(y - radius, 0, self.resolution[0]) : self.clamp(y + radius, 0, self.resolution[0]),
              self.clamp(x - radius, 0, self.resolution[1]): self.clamp(x + radius, 0, self.resolution[1]), :] = color
        return frame
